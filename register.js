...
// app.get('/api/students', async (req, res) => {
//   try {
//     const students = await Student.find({}, 'studentName  studentGrade courses guardianName guardianEmail guardianPhone ');
...
//     if (!username || !email || !studentName || !studentDOB || !studentGrade || 
...
//   if (error) {
//     console.log('❌ Error sending email:', error);
//     return res.status(500).json({ message: 'Student created, but email failed to send.' });
//   }
//   console.log('📤 Email sent:', info.response);
//   res.status(201).json({ message: '✅ Student account created. Email sent with credentials.' });
// });
